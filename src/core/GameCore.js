...
class GameCore {
    ...
    initializeSystems() {
        ...
        this.systems.horror = new HorrorSystem(this, { targetTime: 120, cooldown: 45 });
        ...
        this.addHook('onPelletCollected', (player) => {
            try { this.systems.horror.onPelletCollected(); } catch (e) { console.error('Horror hook error (pellet):', e); }
        });

        this.addHook('onMazeReshuffle', () => {
            try { this.systems.horror.onMapReshuffle(); } catch (e) { console.error('Horror hook error (reshuffle):', e); }
        });

        this.addHook('onLevelComplete', (level) => {
            try { this.systems.horror.onLevelComplete(level); } catch (e) { console.error('Horror hook error (level):', e); }
        });

        // Update Hook
        this.addHook('beforeUpdate', (dt) => {
            try { this.systems.horror.update(dt); } catch (e) { console.error('Horror hook error (update):', e); }
        });

        this.addHook('onGameStart', () => {
            try { this.systems.horror.startRun(); } catch (e) { console.error('Horror hook error (start):', e); }
        });

        this.addHook('onGameOver', () => {
            try { this.systems.horror.endRun(); } catch (e) { console.error('Horror hook error (gameover):', e); }
        });
    ...
    updateIntro(dt, currentTime) {
        ...
        if (this.systems.input.isClicked() && this.systems.ui.isIntroComplete()) {
            this.startGame();
        }
    }
    ...
    updateCountdown(dt, currentTime) {
        ...
        if (this.systems.ui.updateCountdown(dt)) {
    ...
    render(currentTime) {
        ...
        if (this.gameState !== 'intro') {
            this.systems.ui.render(this.ctx, this.gameState, this.getUIData(), currentTime);
        }
    ...
    completeLevel() {
        ...
        this.currentMazeSize = Math.floor(this.baseMazeSize * Math.pow(1.15, this.mazeLevel - 1));
    ...
    gameOver() {
        ...
        this.systems.ui.showGameOver(this.gameScore, this.mazeLevel - 1, this.currentMazeSize);
    ...
    generateNewMaze() {
        const maze = this.systems.maze.generate(this.currentMazeSize, this.currentMazeSize);
    ...
    checkWinCondition() {
        ...
        return playerTileX === goalTileX && playerTileY === goalTileY && !this.transitioningToNext;
    }
    
    checkShuffleTimer() {
        const endTime = new Date();
        const timeDiff = (endTime - this.startDate) / 1000;
    ...
    shuffleMaze() {
        ...
        let attempts = 0;
        while (this.systems.maze.isSolid(currentTileY, currentTileX) && attempts < 10) {
            this.generateNewMaze();
            attempts++;
        }
    ...
    getUIData() {
        return {
            ...
            timeRemaining: this.Wait - Math.round(((new Date() - this.startDate) / 1000) % 60),
